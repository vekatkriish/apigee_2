    ...
    for story in response:
        for task in story.Tasks:
            if task.State == 'Defined':
                taskType = task.Description.split(";")[0].split(":")[1]
                print taskType
                if (taskType == 'createBranch'):
                    branch = task.Description.split(";")[1].split(":")[1]
                    print task.oid, task.Name, task.Notes, branch, task.State, task.FormattedID
                    initstr = "./startPipeline.sh "+pipelineUrl+' '+os.environ['runame']+':'+os.environ['rtoken']+" "+task.Notes+" "+task.FormattedID+" "+branch+" "+'noMerge'
                    print initstr
                    os.system(initstr)
                elif (taskType == 'merge'):
